...
for (i, v) in enumerate(a1);
 print(i, ": ", v, ", "); # Note that Julia index starts at 1 not 0
end;

# map works by applying a function on each value of an array
a2=[]; @time for i=1:5; a2=[a2; sphere_vol(i)] end;
...
r = @spawn rand(3, 3); # spawn is telling a worker to create 3x3 random matrix
println(r)
...
@everywhere function find_n_circle(n_total);
 n_circle=0;
 for i=1:n_total;
  x,y = rand(2)
  if (x^2+y^2<=1);
...
ans=pmap(find_n_circle, [(10^8)/2, (10^8)/2]);
4*sum(ans)/10^8
